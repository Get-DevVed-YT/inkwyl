...
<Link href="/editor">Write a story</Link> or read some here.
      ...
      {!loggedIn && (
       ...
       <div>
           {stories.map((story) => (
            <h3><Link shallow href={story.storyLink}>{story.title}</Link> by <Link shallow href={story.userlink}>{story.username}</Link></h3><div style={{ borderTop: "2px solid #fff ", marginLeft: 20, marginRight: 20 }}></div>
        ))}
       </div>
     </div>
    </div>
